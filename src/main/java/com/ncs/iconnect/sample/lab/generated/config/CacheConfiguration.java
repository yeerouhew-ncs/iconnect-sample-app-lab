...
public class CacheConfiguration {
    ...
    public HazelcastInstance hazelcastInstance(JHipsterProperties jHipsterProperties) {
        ...
        HazelcastInstance hazelCastInstance = Hazelcast.getHazelcastInstanceByName("IconnectSampleAppLab");
        if (hazelCastInstance != null) {
            log.debug("Hazelcast already initialized");
            return hazelCastInstance;
        }
        Config config = new Config();
        config.setInstanceName("IconnectSampleAppLab");
        config.getNetworkConfig().setPort(5701);
        config.getNetworkConfig().setPortAutoIncrement(true);

        // In development, remove multicast auto-configuration
        //if (env.acceptsProfiles(Profiles.of(JHipsterConstants.SPRING_PROFILE_DEVELOPMENT))) {
			System.setProperty("hazelcast.local.localAddress", this.env.getProperty("iconnect.cacheconfig.localhostIpAddress"));
        ...
        config.getMapConfigs().put("com.ncs.iconnect.sample.lab.generated.domain.*", initializeDomainMapConfig(jHipsterProperties));
