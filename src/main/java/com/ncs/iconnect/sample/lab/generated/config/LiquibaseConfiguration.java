...
public class LiquibaseConfiguration {
    ...
    @Bean
    public SpringLiquibase liquibase(@Qualifier("taskExecutors") Executor executor,
            @LiquibaseDataSource ObjectProvider<DataSource> liquibaseDataSource, LiquibaseProperties liquibaseProperties,
            ObjectProvider<DataSource> dataSource, DataSourceProperties dataSourceProperties) throws MalformedURLException {
        ...
        if (env.acceptsProfiles(Profiles.of(JHipsterConstants.SPRING_PROFILE_TEST))) {
            liquibase.setChangeLog(new File("src/test/resources/config/liquibase/master.xml").toURL().toString());
        } else {
